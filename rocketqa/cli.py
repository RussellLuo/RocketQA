...
def train(base_model, train_set, use_cuda, epoch, out, **kwargs):
    ...
    dest_model_checkpoint_path = os.path.join(out, get_model_checkpoint_path(base_model))
    ...
    create_if_not_exists(out, 'config.json', lambda: get_model_config(base_model))
    create_if_not_exists(out, 'zh_config.json', lambda: get_model_zh_config())
    create_if_not_exists(out, 'zh_vocab.txt', lambda: get_model_zh_vocab(base_model))
...
def get_model_zh_vocab(model):
    vocab_filename = os.path.join(os.path.expanduser('~/.rocketqa/'), model, 'zh_vocab.txt')
...
def main():
    ...
    train_parser= subparsers.add_parser('train', help='train or finetune the dual/cross encoder model')
    train_parser.add_argument('base_model', choices=rocketqa.available_models(), help='base model', metavar='base_model')
    train_parser.add_argument('train_set', help='train set')
    train_parser.add_argument('--use-cuda', action='store_true', help='whether to run models on GPU (default: %(default)s)')
    train_parser.add_argument('--epoch', type=int, default=2, help='epoch (default: %(default)s)')
    train_parser.add_argument('--out', default='./models', help='output directory (default: %(default)s)')
    train_parser.add_argument('--save-steps', type=int, default=1000, help='save steps (default: %(default)s)')
    train_parser.add_argument('--learning-rate', type=float, default=1e-5, help='learning rate (default: %(default)s)')
    args = parser.parse_args()

    if args.command == 'train':
        train(args.base_model, args.train_set, args.use_cuda, args.epoch, args.out,
